...
_BUILTIN_SERVERS = {
    ...
    "claude_acp":      ("mcp_servers/claude_acp_bridge.py",      "Built-in: Claude (via subscription)"),
}
...
MCP_DISABLED = os.environ.get("RAVEN_DISABLE_MCP", "").lower() in ("1", "true", "yes")


async def register_builtin_servers(mcp_manager):
    """Connect all built-in MCP servers to the manager."""
    if MCP_DISABLED:
        logger.info("Built-in MCP servers disabled via RAVEN_DISABLE_MCP")
        return
