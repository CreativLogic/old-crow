...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt RAVEN' canonical OpenAI-style messages to native Ollama /api/chat.

    RAVEN carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/CreativLogic/RAVEN")
        h.setdefault("X-OpenRouter-Title", "RAVEN")
    return h
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip RAVEN-only metadata before sending messages to providers.
