...
export const THEMES = {
  dark:       { bg:'#0D1B2A', fg:'#E8F4FF', panel:'#1A2E45', border:'#003380', red:'#8B5CF6' },
...
const LS_KEY = 'raven-theme';
const CUSTOM_THEMES_KEY = 'raven-custom-themes';
...
function deriveSyntaxColors(colors) {
  ...
  const [redH, redS, redL] = hexToHSL(colors.red || '#8B5CF6');
  const isDark = bgL < 50;
...
const ADV_KEYS = [
  ...
  { key: 'brandColor',         css: '--brand-color',       label: 'RAVEN Logo',    group: 'Sidebar' },
...
export function applyColors(colors) {
  ...
  _updateFavicon(colors.red || '#8B5CF6');
}
...
// raven silhouette. Returns the inner SVG markup colored with `fg`.
...
export function initThemeUI() {
  ...
  if (harmonyAccentEl && _harmonyHex) {
    _harmonyHex.textContent = harmonyAccentEl.value || '#00A8FF';
  ...
  if (exportBtnEl) {
    ...
    newExp.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = 'raven_' + (obj.name || 'theme') + '.json';
      a.click();
...
function _initPerlinFlow() {
  ...
  function getBg() { return getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || '#0D1B2A'; }
