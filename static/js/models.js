...
const COLLAPSE_KEY = 'raven-models-collapsed';
const FAVORITES_KEY = 'raven-model-favorites';
const USAGE_KEY = 'raven-model-usage';
const SORT_KEY = 'raven-model-sort';
...
export async function refreshModels(force = false) {
  ...
  try {
    ...
    if (!_cachedItems || _cachedItems.length === 0) {
      ...
      const welcomeSub = document.getElementById('welcome-sub');
      if (welcomeSub) welcomeSub.innerHTML = 'Ready when you are — just ask.';
      const welcomeTip = document.getElementById('welcome-tip');
      if (welcomeTip) {
