...
const _rememberedDockKey = (id) => `raven-modal-remembered-dock-${id}`;
...
function _emitModalOpened(id, modal) {
  try {
    window.dispatchEvent(new CustomEvent('raven:modal-opened', {
      detail: { id, modal },
    }));
  } catch (_) {}
}
...
const _DOCK_STORAGE_KEY = 'raven.mobileDockState.v1';
