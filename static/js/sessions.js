...
let _sortMode = Storage.get('raven-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'RAVEN Chat';
...
const FOLDER_STATE_KEY = 'raven-folder-state';
const FOLDER_ORDER_KEY = 'raven-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to RAVEN kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'RAVEN Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('raven-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('raven-session-sort', mode);
  else Storage.remove('raven-session-sort');
  renderSessionList();
}
