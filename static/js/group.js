...
const GROUP_STATE_KEY = 'raven-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const gids = JSON.parse(localStorage.getItem('raven-group-sessions') || '[]');
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('raven-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
