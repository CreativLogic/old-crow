...
const _ACTION_RECENTS_KEY = 'raven-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'raven-user-actions-recent';
...
export function displayMetrics(messageElement, metrics) {
  ...
  metricsContainer.addEventListener('click', (e) => {
    ...
    const ctxColor = ctxPct >= 85 ? 'var(--red, #8B5CF6)' : ctxPct >= 70 ? '#ff9900' : 'var(--color-muted-alt, #6b7280)';
  ...
  if (ctxPct !== undefined && ctxPct > 0) {
    ...
    const ctxColor = ctxPct >= 85 ? 'var(--red, #8B5CF6)' : ctxPct >= 70 ? '#ff9900' : 'var(--green, #98c379)';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'RAVEN' : shortModel(resolvedModel);
