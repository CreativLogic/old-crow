...
const RECENT_KEY = 'raven-model-recent';
const FAVORITES_KEY = 'raven-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('raven-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('raven-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('raven:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('raven:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `raven-model-favorites[0]`
