...
app = FastAPI(
    title="RAVEN",
    description="Your AI Operating System — self-hosted workspace with chat, agents, memory, research, and multi-modal capabilities",
    version="1.0.0",
)
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. RAVEN's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-RAVEN-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-RAVEN-Owner") or "").strip()
...
async def startup_event():
    ...
    # `RAVEN_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("RAVEN_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (RAVEN_INPROCESS_TASKS=0); "
