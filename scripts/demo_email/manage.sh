...
# a switchable 'Demo' account in RAVEN + fake seed mail.
#
...
DOCKER_DIR="${RAVEN_DEMO_MAIL_DIR:-$HOME/docker/snappymail}"
USERS_FILE="$DOCKER_DIR/dovecot/conf/users"
DEMO_USER="demo@raven.local"
DEMO_PASS="demodemo"
...
case "${1:-}" in
  setup)
    ...
    echo
    echo "Done. In RAVEN, switch to the 'Demo' account to show off the inbox."
    ;;
  reseed)
    "$PY" "$HERE/seed_demo_emails.py" --reset
    ;;
