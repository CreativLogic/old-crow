                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in RAVEN (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in RAVEN" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "raven_kind" if "raven_kind" in cols else "'scheduled' AS raven_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                raven_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-RAVEN-Origin"] = "raven-ui"
                outer["X-RAVEN-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", raven_kind or "scheduled")[:64]
                outer["X-RAVEN-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `raven-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `RAVEN_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `raven-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("RAVEN_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `RAVEN_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (RAVEN_INPROCESS_POLLERS=0); "
            "drive `raven-mail poll-scheduled` externally."
        )
        return
    import asyncio
