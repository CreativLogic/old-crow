...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When RAVEN runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends RAVEN' own interpreter directory to PATH. If RAVEN itself is
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by RAVEN.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$RAVEN_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $RAVEN_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        runner_lines.append('  exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('  exit "$RAVEN_PREFLIGHT_EXIT"')
...
def _append_serve_exit_code_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('RAVEN_CMD_EXIT=$?')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $RAVEN_CMD_EXIT ==="; exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('echo ""; echo "=== Process exited with code $RAVEN_CMD_EXIT ==="')
        runner_lines.append('exit "$RAVEN_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[raven] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('      echo "[raven] CUDA nvcc found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      echo "[raven] WARNING: no HIP/CUDA toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[raven]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[raven]   Install ROCm for AMD GPUs or vLLM/CUDA tooling for NVIDIA, then re-launch this serve task."')
...
WIN_SESSION_DIR = "$env:TEMP\\\\raven-sessions"
