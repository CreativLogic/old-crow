...
# Override at deploy time via RAVEN_MAIL_ATTACHMENTS_DIR. Defaults to a
# subdir of the install's data/ tree so the app works out-of-the-box without
# a hardcoded /home/<user>/ path.
ATTACHMENTS_DIR = Path(os.environ.get("RAVEN_MAIL_ATTACHMENTS_DIR", str(DATA_DIR / "mail-attachments")))
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "raven_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN raven_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("RAVEN_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for RAVEN-generated mail (e.g. reminder, scheduled).
    raven_kind: Optional[str] = None
